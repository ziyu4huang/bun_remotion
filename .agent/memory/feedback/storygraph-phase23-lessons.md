---
name: storygraph-phase23-lessons
description: Lessons from Phase 23 AI Cross-Link Discovery implementation in storygraph
type: feedback
---
...
**Rule:** For storygraph subagent integration, use the file-based JSON I/O pattern (write input → external AI → read output). This matches graphify-check.ts enrichment pattern and keeps the codebase consistent.

**Why:** storygraph has no direct AI API dependency. All AI calls go through Claude Code's Agent tool during skill execution. The file-based pattern writes structured JSON sidecars that subagents can consume.
