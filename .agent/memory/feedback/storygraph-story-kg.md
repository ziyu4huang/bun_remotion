...
**How to apply:** When running `/storygraph` on a Remotion episode series, the primary extraction path is:
...
- `bun_app/storygraph/src/scripts/gen-story-html.ts` — reads graph.json, builds graphology graph, runs Louvain, generates vis.js HTML
- `bun_app/storygraph/src/scripts/graphify-episode.ts` — regex-based narrative extraction (backup/quick mode)
- `bun_app/storygraph/src/scripts/graphify-merge.ts` — combines per-episode graphs with cross-episode link edges
- `bun_app/storygraph/src/scripts/graphify-check.ts` — consistency checking
- `bun_app/storygraph/src/scripts/graphify-pipeline.ts` — orchestrates all steps
