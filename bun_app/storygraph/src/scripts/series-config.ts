...
export interface ScoringProfile {
  /** Node types that receive bonus scoring weight */
  bonusNodeTypes: Array<{ type: string; weight: number }>;
  /** WARN penalty multiplier */
  warnPenalty: number;
  /** FAIL penalty multiplier */
  failPenalty: number;
  /** Arc shape for plot/comedy arc analysis */
...
export interface SeriesConfig {
  ...
  /** Genre classification for genre-aware checks and scoring */
  genre?: StoryGenre;
  /** Scoring profile overrides (default derived from genre) */
  scoringProfile?: ScoringProfile;
  /** Per-genre effect/SFX detection pattern */
...
// ─── Xianxia System Meme (系統文小說梗) ───

export const xianxiaSystemMemeConfig: SeriesConfig = {
  seriesId: "xianxia-system-meme",
  displayName: "系統文小說梗",
  charNames: {
    xiuxiu: "修修",
    system: "系統",
  ...
  traitPatterns: {
    xiuxiu: [
      { pattern: /崩潰|哭|救命|不要|完蛋|死了/, trait: "永遠在崩潰邊緣" },
      { pattern: /系統|任務|抹除|懲罰/, trait: "被系統支配的恐懼" },
      { pattern: /偷懶|摸魚|休息|睡覺/, trait: "只想躺平" },
    ],
    system: [
      { pattern: /任務|完成|失敗|抹除|倒計時/, trait: "任務狂魔" },
      { pattern: /冷靜|理性|計算|分析/, trait: "冷淡AI口吻" },
    ],
    shijie: [
      { pattern: /不錯|繼續|加油|很好/, trait: "溫柔表面" },
      { pattern: /不過|但是|其實|放心/, trait: "腹黑本質" },
    ],
  },
  techPatterns: [
    /系統任務/g, /任務面板/g, /抹除警告/g, /倒計時/g,
    /強制執行/g, /修為/g, /境界/g, /突破/g,
  ],
  gagSource: "plan_md",
  episodeDirPattern: /^xianxia-system-meme-ep(\d+)$/,
  genre: "xianxia_comedy",
};
...
/** Resolve genre from config, defaulting to "generic" */
export function resolveGenre(config: SeriesConfig): StoryGenre {
  return config.genre ?? "generic";
}

/** Resolve the full scoring profile from genre, with defaults */
export function resolveScoringProfile(config: SeriesConfig): ScoringProfile {
  if (config.scoringProfile) return config.scoringProfile;
  ...
  switch (genre) {
    ...
    case "novel_system":
      return {
        bonusNodeTypes: [
          { type: "tech_term", weight: 1 },
          { type: "character_trait", weight: 1 },
        ],
        warnPenalty: 1,
        failPenalty: 2,
        arcShape: "freytag",
    ...
    case "galgame_meme":
      return {
        bonusNodeTypes: [
          { type: "gag_manifestation", weight: 1.5 },
          { type: "character_trait", weight: 1 },
        ],
        warnPenalty: 1,
        failPenalty: 2,
...
/** Get the effect pattern for a config, with genre-aware default */
export function getEffectPattern(config: SeriesConfig): RegExp | null {
  if (config.effectPattern) return config.effectPattern;
  const genre = config.genre ?? "generic";
  switch (genre) {
    case "xianxia_comedy":
    case "novel_system":
      return /闪电|爆炸|轰|砰|咻|嗙|轟|叮|咚|！{2,}|？！/g;
    case "galgame_meme":
      return /哈哈|噗|XD|（笑|笑死|無言|白眼|翻桌|摔|啊|咦|欸{2,}|！{2,}|！！/g;
    default:
      return null;
  }
}

/** Get the title extraction pattern for a config */
export function getTitlePattern(config: SeriesConfig): RegExp | null {
  if (config.titlePattern) return config.titlePattern;
  const genre = config.genre ?? "generic";
  switch (genre) {
    case "xianxia_comedy":
    case "novel_system":
      return /第[一二三四五六七八九十]+章\s*第[一二三四五六七八九十]+集[：:]\s*(.+)/;
    case "galgame_meme":
      return /第[一二三四五六七八九十\d]+集[：:]\s*(.+)/;
    default:
      return null;
  }
}
