...
export function genNarration(ctx: ScaffoldContext): string {
  ...
  const sceneNames = ["TitleScene"];
  for (let i = 1; i <= naming.numContentScenes; i++) {
    sceneNames.push(`${config.contentScenePrefix}Scene${i}`);
  }
  sceneNames.push("OutroScene");
  ...
  return `/**
 * Narration scripts for ${config.displayName} ${naming.chapter !== null ? `第${naming.chapter}章` : ""} 第${naming.episode}集
 *
...
export function genTodoMd(ctx: ScaffoldContext): string {
  ...
  const chapterLabel = naming.chapter !== null
    ...
    : `第 ${naming.episode} 集`;
  const sceneList = ["Title"];
  for (let i = 1; i <= naming.numContentScenes; i++) {
    sceneList.push(`${config.contentScenePrefix}${i}`);
  }
  sceneList.push("Outro");
  const sceneListStr = sceneList.join(", ");

  const contentSceneTasks = Array.from({ length: naming.numContentScenes }, (_, i) => {
    const idx = i + 1;
    return `- [ ] Write src/scenes/${config.contentScenePrefix}Scene${idx}.tsx (TODO: scene description)`;
  }).join("\n");

  return `# TODO — ${config.displayName} ${chapterLabel}
...
${naming.chapter !== null ? `Chapter: ${chapterLabel}` : ""}
...
- [ ] Run sync-images.sh to copy asset images
