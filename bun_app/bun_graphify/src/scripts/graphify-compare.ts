...
Output:
  bun_graphify_out_compare/comparison-report.md
  bun_graphify_out/ restored from best-scoring mode
`);
  process.exit(0);
}
...
const compareDir = resolve(seriesDir, "bun_graphify_out_compare");
...
for (const mode of modesToCompare) {
  ...
  const modeOutDir = resolve(seriesDir, `bun_graphify_out_${mode}`);
...
for (const s of modeStats) {
  ...
  // Total volume bonus (more nodes = more information)
...
report.push(`| WARN penalty | -1 per WARN |`);
report.push(`| FAIL penalty | -2 per FAIL |`);
...
const bestOutDir = resolve(seriesDir, `bun_graphify_out_${best.mode}`);
if (existsSync(outDir)) rmSync(outDir, { recursive: true });
if (existsSync(bestOutDir)) {
  cpSync(bestOutDir, outDir, { recursive: true });
  console.log(`\nRestored ${best.mode.toUpperCase()} output to bun_graphify_out/`);
}
