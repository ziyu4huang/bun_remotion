...
// Load per-segment audio durations (generated by generate-tts.ts)
...
export const MyScene: React.FC = () => {
  ...
  // 1. Current line — uses proportional timing from TTS segment durations
  ...
  // Use proportional timing, NOT equal division
  const rawDurs = segmentDurations[SCENE_NAME];
  const totalSec = rawDurs?.reduce((a, b) => a + b, 0) ?? 1;
  const targetFrame = rawDurs
    ? Math.round(rawDurs.slice(0, 2).reduce((a, b) => a + b, 0) / totalSec * durationInFrames)
    : (2 / dialogLines.length) * durationInFrames;

  return (
    <AbsoluteFill>
      {/* Background */}
      {/* Characters — speaking state from currentLine.character */}
      {/* Effects — triggered by currentLineIndex or targetFrame */}
...
### 4. Use proportional timing from `segment-durations.json`

**Equal division is WRONG for multi-character scenes.** TTS segments have unequal lengths (2s for short lines, 12s for long ones). Equal division causes text-audio mismatch — text changes before/after the audio finishes.

```tsx
// ❌ WRONG — equal division causes text-audio desync
const lineDuration = durationInFrames / dialogLines.length;
const currentLineIndex = Math.floor(frame / lineDuration);

// ✅ CORRECT — proportional to actual audio durations
import { getLineIndex } from "../../../fixture/components/dialogTiming";
const currentLineIndex = getLineIndex(frame, durationInFrames, dialogLines.length, segmentDurations[sceneName]);
```

`generate-tts.ts` writes `segment-durations.json` (raw seconds per segment) alongside `durations.json`. The `getLineIndex` utility allocates frames proportionally and falls back to equal division when data is missing.

**When to use equal division:** Single-voice narration where all lines are roughly the same length (e.g., TitleScene, OutroScene). Multi-character dialog always needs proportional timing.
...
- [debugging](../debugging/debugging.md) — Verification: `remotion still`, brightness check, common bugs
