...
- **Scaffold order**: narration.ts → episode PLAN.md (draft) → **`/storygraph` pipeline + subagent gate** → user approves → TODO.md → configs → scenes → workspace PLAN.md → dev.sh → root pkg → sync-images → bun install → TTS.
