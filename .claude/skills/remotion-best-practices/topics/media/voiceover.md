...
bun_remotion_proj/<my-video>/
  scripts/
    ...
    generate-tts.ts         # TTS generation → writes durations.json + segment-durations.json
  public/
    audio/
      .gitkeep              # tracked
      01-title.mp3          # generated (gitignored)
      durations.json        # per-scene frame counts — commit this
      segment-durations.json # per-scene per-segment raw seconds — for dialog sync
  src/
...
See [galgame](../narrative/galgame.md) for voice selection by provider and per-line switching patterns.
...
async function main() {
  ...
  // Write segment-durations.json — per-scene per-segment raw audio durations (seconds)
  // Used by getLineIndex() in scene components for proportional dialog timing
  // Falls back gracefully to equal division when missing
...
**/public/audio/*.wav
**/public/audio/*.mp3
# durations.json and segment-durations.json are intentionally NOT gitignored —
# commit them so the video builds without regenerating audio on a fresh clone.
```
