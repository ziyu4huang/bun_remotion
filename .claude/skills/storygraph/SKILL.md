...
```bash
bun install --cwd bun_app/storygraph
...
- **Paths must be absolute** — `bun run --cwd bun_app/storygraph` sets CWD for the spawned process only. All `<dir>` arguments must be absolute paths (e.g., `/Users/.../weapon-forger-ch1-ep2`), NOT relative. Relative paths get resolved from `bun_app/storygraph/` and fail silently.
- **storygraph_out/** is in `.gitignore` — output is ephemeral
...
- [../../bun_app/storygraph/PLAN.md](../../bun_app/storygraph/PLAN.md) — Code-level plan + reuse reference
- [../../bun_app/storygraph/TODO.md](../../bun_app/storygraph/TODO.md) — Code-level tasks (file/line specific)
