 ...
 *
 * Voice:  uncle_fu  (warm narrator, Qwen3-TTS default)
 * Speed:  0.97      (slightly slower for financial content comprehension)
 *
 * Usage:
 *   bun run scripts/generate-tts.ts                 # generate all scenes
 *   bun run scripts/generate-tts.ts --skip-existing  # skip already-generated
 *   bun run scripts/generate-tts.ts --scene KLineScene
 *
 * Story spec (for full preview audiobook):
 *   scripts/stv.sh produce mlx_tts/stories/taiwan_stock_market.story.json
 */
...
// ─── WAV duration from header (works for both mlx_tts and Gemini output) ──────
function wavDurationFrames(filePath: string, fps: number): number {
  const buf = readFileSync(filePath);
  const byteRate = buf.readUInt32LE(28);
  const dataSize = buf.readUInt32LE(40);
  return Math.ceil((dataSize / byteRate) * fps) + 15; // +15 frame trailing buffer
}
...
async function main() {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    // Small pause between mlx_tts runs to let GPU memory settle
  ...
  // Write durations.json so Root.tsx can read scene lengths without Node.js imports
  ...
  if (useMlxTts) {
    console.log(`\nTip: produce a full preview audiobook with:`);
    console.log(`  scripts/stv.sh produce mlx_tts/stories/taiwan_stock_market.story.json`);
  }
}
