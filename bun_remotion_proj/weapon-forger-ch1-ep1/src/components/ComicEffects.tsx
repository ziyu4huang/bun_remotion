...
const SweatEffect: React.FC<{ frame: number }> = ({ frame }) => {
  const dropY = interpolate(frame, [0, 40], [-10, 30], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const opacity = interpolate(frame, [0, 5, 35, 45], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const wobble = Math.sin(frame * 0.15) * 5;
  return (
    <div style={{ transform: `translateY(${dropY}px) translateX(${wobble}px)`, opacity }}>
      <div style={{
        width: 28, height: 36,
...
const HeartEffect: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    <div style={{
      ...
      opacity, filter: "drop-shadow(0 2px 6px rgba(244, 63, 94, 0.5))",
    }}>❤️</div>
  );
};
...
const DotsEffect: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    <div style={{ display: "flex", gap: 12, marginTop: 5 }}>
      {dots.map((i) => {
        const dotOpacity = interpolate(frame,
          [baseDelay + i * 8, baseDelay + i * 8 + 6, baseDelay + i * 8 + 30, baseDelay + i * 8 + 40],
          [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
        const dotBounce = interpolate(Math.sin((frame - baseDelay - i * 8) * 0.2), [-1, 1], [-3, 3]);
        return (
          <div key={i} style={{
  ...
  return (
    <div style={{ display: "flex", gap: 20, opacity }}>
      <div style={{
        ...
        opacity: interpolate(tearY, [0, 10, 35], [0, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
      }} />
      <div style={{
        width: 8, height: 18,
        borderRadius: "50% 50% 50% 50% / 60% 60% 40% 40%",
        ...
        opacity: interpolate(tearY + 5, [0, 10, 35], [0, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
      }} />
      <div style={{ fontSize: 36, transform: `translateY(${Math.sin(frame * 0.08) * 3}px)` }}>😢</div>
    </div>
  );
};
