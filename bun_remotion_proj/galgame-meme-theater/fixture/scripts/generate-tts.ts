...
// Import narration from episode's scripts/narration.ts
const { narrations } = require(join(APP_DIR, "scripts", "narration.ts")) as {
  narrations: { scene: string; file: string; text: string }[];
};

// ─── mlx_tts config (macOS) ───────────────────────────────────────────────────
const MLX_TTS_ROOT = join(REPO_ROOT, "mlx_tts");
const MLX_PYTHON = join(MLX_TTS_ROOT, ".venv", "bin", "python");
const MLX_VOICE = "serena";
